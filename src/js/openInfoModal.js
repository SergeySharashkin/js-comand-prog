...
export function openInfoModal(e) {
  ...
  const { popularity, url, alt, id, overview, rating, count, original, genres } = data;
  currentId = id;
  getTrailerUrl(id);
  ...
  if (localStorage.getItem('savedStorage')) {
    let savedJson = localStorage.getItem('watchedStorage');
  ...
  <div class="modal__table-wrap">
    ...
    <div class="modal-add-btns">
      <button class="add-btn trailer-btn">Trailer</button>
  ...
  watchedBtn.addEventListener('click', () => {
    ...
    if (!FilmsID.includes(currentId)) {
      // if (!librarySection.find(film => film.id === currentId)) {
    ...
    return console.log(filterFilms);
  });
